...
class SettingsWindow:
    def __init__(self, parent, settings_port: SettingsPort, on_theme_change: Callable[[str], None] = None):
        self.parent = parent
    ...
    def _setup_ui(self):
        ...
        # Generate UI dynamically from schema
        for category in self.schema_adapter.get_categories():
    ...
    def _create_setting_control(self, parent, setting, row):
        ...
        if setting_type == "checkbox":
            ...
            control = ttk.Combobox(parent, textvariable=var, values=options, state="readonly", width=20)
            control.grid(row=row, column=1, sticky=(tk.W, tk.E), pady=5, padx=(10, 0))
            self.setting_vars[key] = var
            
            # Special handling for theme changes
            if key == "theme" and self.on_theme_change:
                control.bind("<<ComboboxSelected>>", lambda e, k=key: self._on_setting_change(k))
                
        elif setting_type == "slider":
            var = tk.IntVar(value=current_value)
            min_val = setting.get("min", 0)
            max_val = setting.get("max", 100)
            control = tk.Scale(parent, variable=var, from_=min_val, to=max_val, orient=tk.HORIZONTAL, length=200)
            control.grid(row=row, column=1, sticky=(tk.W, tk.E), pady=5, padx=(10, 0))
            self.setting_vars[key] = var
            ...
            control = tk.Spinbox(parent, textvariable=var, from_=min_val, to=max_val, increment=step, width=10, state="readonly")
            control.grid(row=row, column=1, sticky=tk.W, pady=5, padx=(10, 0))
            self.setting_vars[key] = var
    ...
    def _auto_size_and_center(self):
        # Обновляем окно для получения правильных размеров
        self.window.update_idletasks()
        
        # Получаем необходимые размеры окна на основе содержимого
    ...
    def _reset_to_default(self):
        ...
        for key, default_value in defaults.items():
            if key in self.setting_vars:
                ...
                # Special handling for theme preview
                if key == "theme" and self.on_theme_change:
                    try:
                        self.on_theme_change(default_value)
                    except Exception as e:
                        logger.error(f"Failed to preview default theme {default_value}: {e}")
